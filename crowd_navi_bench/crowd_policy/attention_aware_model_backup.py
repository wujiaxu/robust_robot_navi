...
def time_to_collision_with_segment(position_robot, velocity_robot, p1, p2):
    # Define robot's initial position and velocity
...
def cal_steering(position_robot,velo_robot,goal_robot,r_robot,others,walls,beta=3.0):
    ...
    Eh = np.inf
    alpha_des = 0
    for alpha in np.arange(-90,0.1,90.1):
        self_steering_angle = alpha*np.pi/180.
        robot_theta = np.arctan2(velo_robot[1],velo_robot[0])+self_steering_angle
        velo_robot_steered = np.linalg.norm(velo_robot)*np.array([np.cos(robot_theta),
                                                                np.sin(robot_theta)])
        ...
        for wall in walls:
            p1, p2 = wall
            ttc_wall = time_to_collision_with_segment(position_robot, velo_robot_steered, p1, p2)
            dist_to_wall = ttc_wall*V0
            print("d wall",dist_to_wall)
...
if __name__ == "__main__":
    ...
    for i in range(50):
        ...
        for human in env.crowd:
            ...
            alpha_des,Eh = cal_steering(position_robot,velo_robot,goal_robot,r_robot,others,walls)  
