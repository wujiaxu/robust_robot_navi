...
class VAE(nn.Module):
    def __init__(self, input_dim = 130, device="cuda:0", latent_dim=10,learning_rate=1e-4,hidden_size=64):
    ...
    def estimate_log_probability(self, input_data, num_samples=100,plot_recon=False):
        ...
        for _ in range(num_samples):
            ...
            if plot_recon:
                plt.plot(np.linspace(0,130,130),input_data.cpu().numpy()[0],color="b")
                plt.plot(np.linspace(0,130,130),recon_x.cpu().numpy()[0],color="r")
        ...
        if (epoch+1) % 100 == 0:
            ...
            with torch.no_grad():
                for batch_data in dataloader:
                    ...
                    for i in range(5):
                        plt.plot(np.linspace(0,130,130),batch_data[i],color="b")
                        plt.plot(np.linspace(0,130,130),reconstruction[i],color="r")
                        plt.savefig("density_model/{}.png".format(i))
                        plt.close()
                        plt.clf()
                    break
            torch.save(vae.state_dict(), 'density_model/vae_model_{}.pth'.format(epoch))
            vae.train()


if __name__ == "__main__":
    print("train vae")
    dataset = DataRecorder(save_dir="/home/dl/wu_ws/HARL/crowd_navi_bench/results/crowd_env/crowd_navi/robot_crowd_happo/train_on_ai_090_4p_3c_rvs_room361_vs_c090_happo_5p_3c_rvs_room361_data/seed-00001-2024-10-01-03-28-09/logs")
    dataset.load_data()
