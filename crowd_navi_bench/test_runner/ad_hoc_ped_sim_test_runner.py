            ...
            for e in range(self.algo_args["render"]["render_episodes"]):
                for v in range(10):
                    ...
                    if self.args["human_policy"] == "ORCA":
                        h_types = []
                        for i in range(self.envs.n_agents):
                            h_types.append(np.random.choice(list(self.human_types))) 
                    eval_obs, _, eval_available_actions = self.envs.reset(seed=e)
                    ...
                    while True:
                        ...
                        for agent_id in range(self.envs.n_agents):
                            if self.args["human_policy"] == "ORCA":
                    ...
                    self.video_recorder.save(
                        ...
                        e,v,''.join(map(str, np.array([h_type.value for h_type in h_types])))),
                        save_pdf=True)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exp_name", type=str, default="ptocra_crowdsim_5p_rvs_circlecross", help="Experiment name."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--model_dir",
        type=str,
        default="/home/dl/wu_ws/HARL/results_seed_1/crowd_env/crowd_navi/robot_crowd_happo/c0.90_happo_5p_3c_rvs_circlecross/seed-00001-2024-09-27-10-55-42",
