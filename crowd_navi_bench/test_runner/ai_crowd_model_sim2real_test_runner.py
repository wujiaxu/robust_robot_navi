    ...
    def render(self):
        ...
        if self.manual_expand_dims:
            ...
            for e in range(self.algo_args["render"]["render_episodes"]):
                ...
                eval_obs, _, eval_available_actions = self.envs.reset(seed=-7,
                                                                      random_attribute_seed=0,
                ...
                # save video 
                self.video_recorder.save("test_episode_{}_{}_{}.mp4".format(
                    e,eval_infos[0]["episode_info"],
                    ''.join(map(str, preference))),save_pdf=True)
        else:
...
if __name__ == "__main__":
    ...
    env_args["human_preference_vector_dim"] = human_model_all_config["env_args"]["human_preference_vector_dim"]
    # print(env_args["human_preference_vector_dim"])
