...
class BaseLogger:
    ...
    def init(self, episodes):
        ...
        self.train_episode_aux_rewards = np.zeros(
            self.algo_args["train"]["n_rollout_threads"]
        )
        self.done_episodes_aux_rewards = []
    ...
    def per_step(self, data):
        ...
        aux_reward_env = np.mean(aux_rewards[:,1:],axis=1).flatten()
        self.train_episode_rewards += reward_env
        self.train_episode_aux_rewards += aux_reward_env
        for t in range(self.algo_args["train"]["n_rollout_threads"]):
            if dones_env[t]:
                ...
                self.done_episodes_aux_rewards.append(self.train_episode_aux_rewards[t])
                self.train_episode_aux_rewards[t] = 0

    def episode_log(
        self, actor_train_infos, critic_train_info, actor_buffer, critic_buffer,discrim_train_info=None
    ):
        ...
        self.log_train(actor_train_infos, critic_train_info,discrim_train_info)
        ...
        if len(self.done_episodes_rewards) > 0:
            ...
            aver_episode_aux_rewards = np.mean(self.done_episodes_aux_rewards)
            print(
                "Some episodes done, average episode reward is {}, aux_reward is {}.\n".format(
                    aver_episode_rewards,aver_episode_aux_rewards
                )
            )
            ...
            self.done_episodes_rewards = []
            self.writter.add_scalars(
                "train_episode_aux_rewards",
                {"aver_rewards": aver_episode_aux_rewards},
                self.total_num_steps,
            )
            self.done_episodes_aux_rewards = []
    ...
    def log_train(self, actor_train_infos, critic_train_info,discrim_train_info=None):
        ...
        if discrim_train_info is not None:
            for k, v in discrim_train_info.items():
                discrim_k = "discriminator/"+k
                self.writter.add_scalars(discrim_k, {discrim_k:v},self.total_num_steps)
