...
from harl.utils.trans_tools import _flatten, _sa_cast, _ma_cast


class OnPolicyCriticBufferCrowdEP:
    """On-policy buffer for critic that uses Feature-Pruned (EP) state.
    shared_obs --> crowd reward, 
    """
    ...
    def recurrent_generator_critic(self, critic_num_mini_batch, data_chunk_length):
        ...
        # (episode_length + 1, n_rollout_threads, *share_obs_shape) --> (episode_length, n_rollout_threads, *share_obs_shape)
        # --> (n_rollout_threads, episode_length, *share_obs_shape) --> (n_rollout_threads * episode_length, *share_obs_shape)
        # (R*L,N*share_obs_shape)
        share_obs = _sa_cast(self.share_obs[:-1])

        # value.transpose(1, 2, 0, 3).reshape(-1, *value.shape[3:])
        # (episode_length + 1, n_rollout_threads, num_agents, 1), --> (episode_length, n_rollout_threads, num_agents, 1),
        # --> (R,L,N,1)--> (R*L,N,1)
