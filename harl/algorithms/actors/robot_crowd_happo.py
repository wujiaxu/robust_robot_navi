...
class RobotCrowdHAPPO(OnPolicyBase):
    def __init__(self, args, obs_space, act_space, device=torch.device("cpu")):
        ...
        if self.use_discriminator:
            from harl.models.discriminator_models.behavior_discriminator import BehaviorDiscriminator
            self.discrim = BehaviorDiscriminator(
                self.actor.hidden_sizes[-1]*self.actor.recurrent_n+2,
                self.human_preference_dim,
                [128,128],
                args["initialization_method"],
                device
            )
            parameters+=list(self.discrim.parameters())
        else:
            self.discrim = None
    ...
    def get_discriminator_loss(self,obs,
                               rnn_states,
                               actions,
        ...
        if self.use_policy_active_masks:
            loss = (self.discrim(obs,rnn_states,actions,active_masks_batch)
                ).sum()/active_masks_batch.sum()
        else:
            loss = self.discrim(obs,rnn_states,actions).mean()
    ...
    def get_aux_reward(self,obs,
        ...
        return self.discrim(obs,rnn_states,actions,active_masks)
    ...
    def update(self, sample):
        ...
        if self.use_discriminator:
            discrim_loss = self.get_discriminator_loss(obs_batch,
                                                rnn_states,
                                                actions_batch,
                                                active_masks_batch)
            (policy_loss +discrim_loss*self.discrim_loss_scale - dist_entropy * self.entropy_coef).backward()
        else:
            discrim_loss = None
        ...
        return policy_loss, discrim_loss, dist_entropy, actor_grad_norm, imp_weights
    ...
    def train(self, actor_buffer, advantages, state_type):
        ...
        train_info["ratio"] = 0
        if self.use_discriminator:
            train_info["discriminator_loss"] = 0
        ...
        for _ in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                policy_loss, discrim_loss, dist_entropy, actor_grad_norm, imp_weights = self.update(
                    sample
                )
                if self.use_discriminator:
                    train_info["discriminator_loss"]+=discrim_loss.item()
