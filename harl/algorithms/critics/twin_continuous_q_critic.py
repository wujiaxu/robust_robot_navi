...
class TwinContinuousQCritic:
    ...
    def train(
        ...
        share_obs = check(share_obs).to(**self.tpdv)
        actions = check(actions).to(**self.tpdv).squeeze(0)
        ...
        next_actions = next_actions.to(**self.tpdv).squeeze(0)
        # next_actions = torch.cat([next_actions[i] for i in range(next_actions.shape[0])], dim=-1)
        ...
        if torch.sum(torch.isnan(q_targets))>0:
            raise ValueError
    ...
    def train_ldm(
        ...
        next_q_values = torch.min(next_q_values1, next_q_values2)
        if self.use_proper_time_limits:
            # print(reward.shape,next_q_values.shape)
            q_targets = torch.max(reward ,gamma * next_q_values * (1 - term))
        else:
            q_targets = torch.max(reward , gamma * next_q_values * (1 - done))
        # print("ldm",q_targets.shape, self.critic(share_obs, actions).shape)
        critic_loss1 = torch.mean(
            torch.nn.functional.mse_loss(self.critic(share_obs, actions), q_targets)
        )
        critic_loss2 = torch.mean(
