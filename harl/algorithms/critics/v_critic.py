...
from harl.models.value_function_models.custom_v_net import CustomVNet,CustomDecVNet,CustomDoubleVNet,CustomDoubleDecVNet
...
class VCritic:
    ...
    def __init__(self, args, cent_obs_space, device=torch.device("cpu")):
        ...
        if "custom" in args.keys() and args["custom"]:
            self.critic = CustomVNet(args, self.share_obs_space, self.device)
        else:
...
class DecVCritic:
    ...
    def __init__(self, args, obs_space, device=torch.device("cpu")):
        ...
        self.critic = CustomDecVNet(args, self.obs_space, self.device)
...
class DoubleVCritic:
    ...
    def __init__(self, args, cent_obs_space, device=torch.device("cpu")):
        ...
        self.critic = CustomDoubleVNet(args, self.share_obs_space, self.device)
    ...
    def get_values(self, cent_obs, rnn_states_critic, masks):
        ...
        Returns:
            ...
            rnn_states_critic: (torch.Tensor) updated critic network RNN states.
        """
        values, aux_values, rnn_states_critic = self.critic(cent_obs, rnn_states_critic, masks)
        return values, aux_values, rnn_states_critic
    ...
    def update(self, sample, value_normalizer=None,aux_value_normalizer=None):
        """Update critic network.
        Args:
            sample: (Tuple) contains data batch with which to update networks.
            value_normalizer: (ValueNorm) normalize the rewards, denormalize critic outputs.
        ...
        value_preds_batch = check(value_preds_batch).to(**self.tpdv)
        return_batch = check(return_batch).to(**self.tpdv)
        aux_value_preds_batch = check(aux_value_preds_batch).to(**self.tpdv)
        aux_return_batch = check(aux_return_batch).to(**self.tpdv)

        values, aux_values, _ = self.get_values(
            share_obs_batch, rnn_states_critic_batch, masks_batch
        )
        ...
        aux_value_loss = self.cal_value_loss(
            aux_values, aux_value_preds_batch, aux_return_batch, value_normalizer=aux_value_normalizer
        )

        self.critic_optimizer.zero_grad()

        ((aux_value_loss+value_loss) * self.value_loss_coef).backward()
        ...
        return value_loss, aux_value_loss,critic_grad_norm

    def train(self, critic_buffer, value_normalizer=None,aux_value_normalizer=None):
        """Perform a training update using minibatch GD.
        Args:
            critic_buffer: (OnPolicyCriticBufferEP or OnPolicyCriticBufferFP) buffer containing training data related to critic.
            value_normalizer: (ValueNorm) normalize the rewards, denormalize critic outputs.
        ...
        for _ in range(self.critic_epoch):
            ...
            for sample in data_generator:
                value_loss, aux_value_loss, critic_grad_norm = self.update(
                    sample, value_normalizer=value_normalizer, 
                    aux_value_normalizer=aux_value_normalizer
                )

                train_info["value_loss"] += value_loss.item()
                train_info["critic_grad_norm"] += critic_grad_norm
                train_info["aux_value_loss"] += aux_value_loss.item()
...
class DoubleDecVCritic:
    ...
    def __init__(self, args, obs_space, device=torch.device("cpu")):
        ...
        self.critic = CustomDoubleDecVNet(args, self.obs_space, self.device)
    ...
    def get_values(self, obs, rnn_states_critic, masks):
        ...
        Returns:
            ...
            rnn_states_critic: (torch.Tensor) updated critic network RNN states.
        """
        values, aux_values, rnn_states_critic = self.critic(obs, rnn_states_critic, masks)
        return values, aux_values, rnn_states_critic
    ...
    def update(self, sample, value_normalizer=None,aux_value_normalizer=None):
        ...
        values, aux_values, _ = self.get_values(
            obs_batch, rnn_states_critic_batch, masks_batch
        )
