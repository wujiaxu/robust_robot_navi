...
class OnPolicyBaseRunner:
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        if self.share_param and not("use_discriminator" in algo_args["algo"].keys() and algo_args["algo"]["use_discriminator"]):
            ...
            # self.env_args["human_fov"] = 360 out of date
            print("share param")
        self.use_vis_aware = False
        if args["env"] == "crowd_env_vis":
            algo_args["algo"]["use_vis_aware"] = True
            # self.use_discriminator = False
            self.use_vis_aware = True
        ...
        if self.share_param:
            ...
            if self.robot_num>0:
                ...
                for agent_id in range(self.robot_num):
                    self.actor.append(robot)
                    print(agent_id,"robot")
            current_agent_id = self.robot_num
            if self.use_vis_aware:
                distracted_human_num = env_args.get("distracted_human_num",1)
                distracted_human_agent = ALGO_REGISTRY[args["algo"]](
                    {**algo_args["model"], **algo_args["algo"]},
                    self.envs.observation_space[agent_id],
                    self.envs.action_space[agent_id],
                    device=self.device,
                )
                for _ in range(distracted_human_num):
                    print(current_agent_id,"distracted")
                    self.actor.append(distracted_human_agent) 
                    current_agent_id+=1
            ...
            for agent_id in range(current_agent_id, self.num_agents):
                print(agent_id,"human")
                assert (
                    self.envs.observation_space[agent_id]
                    == self.envs.observation_space[current_agent_id]
                ), "Agents have heterogeneous observation spaces, parameter sharing is not valid."
                assert (
                    self.envs.action_space[agent_id] == self.envs.action_space[current_agent_id]
            ...
            if self.use_vis_aware:
                ...
                for _ in range(distracted_human_num):
                    print(agent_id,"distracted")
                    self.actor.append(distracted_human_agent) 
                    agent_id+=1
            else:
                distracted_human_num = 0
            ...
            for agent_id in range(self.robot_num+1+distracted_human_num,self.num_agents):
                assert (
                    self.envs.observation_space[agent_id]
                ...
                self.actor.append(self.actor[self.robot_num+distracted_human_num])
                print(agent_id,"use discriminator:",human_agent.use_discriminator)
        else:
            self.actor = []
