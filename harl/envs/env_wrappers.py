...
def shareworker(remote, parent_remote, env_fn_wrapper,human_policy_class=None):
    parent_remote.close()
    env = env_fn_wrapper.x()
    if human_policy_class is not None:
        human_policy = human_policy_class(env)
    else:
        human_policy = None
    while True:
        ...
        if cmd == "step":
            if human_policy is not None:
                for agent_id in range(1,env.n_agents):
                    action = human_policy.predict(agent_id)
                    data[agent_id] = action
...
class ShareSubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None,human_policy_class=None):
        """
        ...
        self.ps = [
            Process(
                target=shareworker,
                args=(work_remote, remote, CloudpickleWrapper(env_fn),human_policy_class),
            )
