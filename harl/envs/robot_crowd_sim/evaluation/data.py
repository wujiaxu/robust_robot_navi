...
class RawData(object):
    ...
    def load_trajectory_data(self, data_path):
        ...
        # print(meta_data)
        self.meta_data = meta_data
        self.raw_destinations = destinations
        self.start_step = min([u[0][-1] for u in trajectories])
        self.num_steps = max([u[-1][-1] for u in trajectories]) + 1
        print(self.num_steps,min([u[0][-1] for u in trajectories]))
        ...
        mask_p = torch.zeros((self.num_steps, self.num_pedestrians),dtype=torch.bool)
        mask_v = torch.zeros((self.num_steps, self.num_pedestrians),dtype=torch.bool)
        mask_a = torch.zeros((self.num_steps, self.num_pedestrians),dtype=torch.bool)
        ...
        for i, relays in enumerate(destinations):
            # print(len(relays))# one way point (destination)
        ...
        self.time_unit = meta_data['time_unit']
        # print(self.position)
        print("max ped in frame",torch.max(torch.sum(self.mask_p,dim=1)),torch.argmax(torch.sum(self.mask_p,dim=1)))
        # input()
        # self.ped_num_each_frame = mask_p.
        # print(self.obstacles) #use obstacle:
...
class Pedestrians(object):
    ...
    def calculate_collision_label(ped_features):
        ...
        with torch.no_grad():
            ...
            collisions[collisions >= 0.3] = 0
            collisions[(collisions < 0.3) & (collisions != 0)] = 1
